...
class SeqNN(nn.Module):
    ...
    def __init__(self,
                ...
                final_ch=18,
                bn_momentum=0.1,
                to_extract=[], # indices of the layers (except the last) whose outputs enter the classifier
                pool_dim=4,
                standalone=True):
        super().__init__()
    ...
    def forward(self, x):
        ...
        x = self.mapper(f).flatten(1)
        f = F.adaptive_max_pool1d(f, self.pool_dim).flatten(1) # pools agains overfitting
